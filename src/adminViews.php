...
class adminViews {
    ...
    function static_page_settings() {
        ...
        ?>
        <div class="wrap">
            <h2>Settings of Static Page Generator</h2><hr>
            <form id="staticPageOptions"> 
                <div style='width:60%'>
                    <h3>General Settings</h3><!-- comment -->
                    ...
                    <div class="input-wrap custom-slug-input">
                        ...
                        <label>
                            <input type="checkbox" value="1" <?php echo $options['custom_slug_enable'] == '1' ? 'checked' : "" ?> name="data[custom_slug_enable]">
                            Enable
                        </label>
                    </div><!-- comment -->
                    <hr>
                    <label><input type="checkbox" value="1" <?php echo $options['quick_mood'] == '1' ? 'checked' : "" ?> name="data[quick_mood]">Quick Mood</label>&nbsp;&nbsp; |
                    <input type="hidden" value="0" name="data[listGmood]">
                    &nbsp;&nbsp;<label><input type="checkbox" name="data[listGmood]" <?php echo $options['listGmood'] == '1' ? 'checked' : "" ?> value="1"> Generate Mode(List) </label>
                     <!--<input type="hidden" value="0" name="data[gmood]">;<label>Generate Mode <input type="checkbox" name="data[gmood]"  value="1"></label>-->
                    <hr>
                    <h3>Quick Link Generate</h3><!-- comment -->
                    <label class='description'>ID|find:replace,find2:replace2|slug</label>
                    <textarea id="quickLinkInfo" name="data[quick_link_info]" class="quickLinkInfo"><?php echo $options['quick_link_info'] ?></textarea>
        ...
        <script>
            (function (_) {
                _("#staticPageOptions").on('submit', function (e) {
                    ...
                    _.post(ajaxurl, {action: 'staticPageOptionsStore', formdata: formData}, function (response) {
            ...
            function generateStaticSitemap(_this) {
                (function (_) {
                    ...
                    _.post(ajaxurl, {action: 'generateStaticSitemap'}, function (response) {
            ...
            function deleteSitemaps(_this) {
                ...
                _.post(ajaxurl, {action: 'deleteSitemaps'}, function (response) {
                    ...
                    <div class="keyword-manage">
                        <div class="keyword-groups">
                            <span class="newKey4Static">
                                <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512"><title>Add New</title><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32" d="M256 112v288M400 256H112"/></svg>
                                Add New
                            </span>
                        ...
                        <div class="keyword-keys">
                            ...
                            <button class="update-keyData" type="button">
                                <svg xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512"><title>Update</title><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="32" d="M416 128L192 384l-96-96"/></svg>
                            </button>
                        </div>
                    </div>
                </div>
